...
> 对应 [ROADMAP.md](../ROADMAP.md) 阶段 3.1–3.2。  
> 前置：Docker、OpenSandbox Server、`docker/build.sh` 已构建 `allox/aio-custom:v1`。
...
| 项 | 命令 / 路径 | 预期 | 结果 |
|----|-------------|------|------|
| Dockerfile | `docker/Dockerfile` | `FROM ghcr.io/agent-infra/sandbox:<pin>` | |
...
# 指定镜像跑集成用例
export ALLOX_CUSTOM_IMAGE=allox/aio-custom:v1
