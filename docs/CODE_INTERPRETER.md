...
Allox 仓库内可提供 CI  job（阶段 4 可选）：在 `docker/build.sh` 模式上为 code-interpreter 增加 `.github/workflows/` 或文档化调用上游 `build.sh`。
...
| 示例 profile | `allox-cli/examples/profiles/code.toml.example` |
