"""Prepare the cgroup boundary owned by the trusted Allox OS Guest."""
...
def _cgroup2_mount(root: Path, mounts: list[tuple[Path, str, set[str]]]):
    candidates = [
        item
        for item in mounts
        if item[1] == "cgroup2" and (root == item[0] or item[0] in root.parents)
    ]
...
def prepare_session_runtime(
    *,
    ...
    """Prepare the cgroup v2 subtree used for Session ownership and termination."""
    ...
    def remount_cgroup2() -> None:
        runner(["mount", "-o", "remount,rw", str(cgroup_mount)], check=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = prepare_session_runtime(cgroup_root=args.cgroup_root)
